...
async function check() {
  const { data: o1 } = await supabase.from('stores_orders').select('id, store_id, customer').in('id', [1, 3, 4, 7]);
  console.log("Orders 1,3,4,7:", o1);

  const { data: o17 } = await supabase.from('stores_orders').select('id, store_id, customer').eq('id', 17);
